...
class Configuration extends Component {
  ...
  newSubmit = () => {
    ...
    else {Api.post('configuration/changeSystemTime', body);
  ...
  render() {
    return (
      ...
      <hr className="my-2" style={{paddingTop: 50, paddingBottom: 50}} />
      <Row>
        <Col sm='1'>Hour</Col>
        <Col sm='3'><Input defaultValue={moment().format('HH')} onChange={(e) => this.setState({hour: e.target.value})}/></Col>
        <Col sm='1'>Min.</Col>
        <Col sm='3'><Input defaultValue={moment().format('mm')} onChange={(e) => this.setState({min: e.target.value})}/></Col>
        <Col sm='1'>Second</Col>
        <Col sm='3'><Input defaultValue={moment().format('ss')} onChange={(e) => this.setState({sec: e.target.value})}/></Col>
        </Row><Row style={{paddingTop: 40}}>
        <Col sm='1'>Day</Col>
        <Col sm='3'><Input defaultValue={moment().format('DD')} onChange={(e) => this.setState({day: e.target.value})}/></Col>
        <Col sm='1'>Month</Col>
        <Col sm='3'><Input defaultValue={moment().format('MM')} onChange={(e) => this.setState({month: e.target.value})}/></Col>
        <Col sm='1'>Year</Col>
        <Col sm='3'><Input defaultValue={moment().format('YYYY')} onChange={(e) => this.setState({year: e.target.value})}/></Col>
      </Row>
      <Row style={{ paddingTop: 30}}><Col align='center'><Button type="submit" size="lg" color="primary" 
              onClick={() => {if(window.confirm('Are you sure?')){this.newSubmit()};}}>
              Save
            </Button></Col>
      </Row>
      </Jumbotron>
